...
class GitHubCopilotUsageConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reauth(
        ...
        """Handle re-authentication when GitHub rejects the stored token."""
    ...
    async def async_step_reauth_confirm(
        ...
        """Prompt for a fresh token and validate it.

        A personal access token works regardless of how the entry was
        originally created (PAT or device flow), so reauth always accepts one.
        """
        errors: dict[str, str] = {}
        ...
        if user_input is not None:
            ...
            try:
                await validate_input(self.hass, {CONF_TOKEN: token})
            except GitHubCopilotUsageAuthenticationError:
                errors["base"] = "invalid_auth"
            except GitHubCopilotUsageApiError:
                errors["base"] = "cannot_connect"
                ...
                _LOGGER.exception(
                    "Unexpected exception while validating GitHub Copilot Usage reauth"
                )
                errors["base"] = "unknown"
            else:
